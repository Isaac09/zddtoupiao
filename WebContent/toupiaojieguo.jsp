...
response.setHeader("refresh", "1;URL=InfoServlet?flag=clicktitle&infoid="+infoid);//这里的3,是你要确定的时间秒URL是要跳转的地址

%>

				投票成功！一秒后跳转至<a href="InfoServlet?flag=clicktitle&infoid=${requestScope.infoid }">投票结果页面</a>
			</c:if>
			<c:if test="${requestScope.mark==0}">
				<font color="red">投票失败！你已经投过票了！</font>你可以去查看<a href="InfoServlet?flag=clicktitle&infoid=${requestScope.infoid }">投票结果页面</a>，或者<a href="javascript:window.history.back(-1)">返回</a>
			</c:if>
	</div>
</div>
